...
void
score_add_draw(struct food *f)
{
	SetConsoleTextAttribute(h, FG_LIGHTGRAY | 0);
	
	for (int i = 0; i < 6; i++) {
		SetConsoleCursorPosition(h, (COORD) {WIN_SIZE_X - 7 + i, BOARD_TOP});
		printf("%c", ' ');
	}

	SetConsoleTextAttribute(h, FG_LIGHTGREEN | 0);
	SetConsoleCursorPosition(h, (COORD) {WIN_SIZE_X - 5, BOARD_TOP});
	printf("%4d", f->score);
	SetConsoleCursorPosition(h, (COORD) {WIN_SIZE_X - digits_count(f->score) - 2, BOARD_TOP});
	printf("%c", '+');
	SetConsoleTextAttribute(h, FG_LIGHTGRAY | 0);
}
